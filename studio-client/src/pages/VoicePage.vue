...
<template>
  <div class="p-6 max-w-5xl mx-auto space-y-6">
    <header>
      ...
      <p class="text-[var(--color-ide-text-dim)] mt-2">文字转语音 (TTS) · 语音转文字 (STT)</p>
    </header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-gray-900/50 rounded-2xl p-6 border border-gray-800/50 space-y-4">
        ...
        <textarea v-model="ttsText" rows="4" class="w-full bg-gray-800 border border-gray-700/50 rounded-xl p-3 text-sm text-[var(--color-ide-text)] outline-none focus:border-brand-500/50 resize-none" placeholder="输入要合成的文字..."></textarea>
        <div class="grid grid-cols-2 gap-3">
          <label class="space-y-1">
            <span class="text-xs text-[var(--color-ide-text-dim)]">语音</span>
            <select v-model="ttsVoice" class="w-full bg-gray-800 border border-gray-700/50 rounded-xl px-3 py-2 text-sm text-[var(--color-ide-text)] outline-none">
              <option v-for="v in allVoices" :key="v.id" :value="v.id">{{ v.name }} ({{ v.provider }})</option>
            </select>
          </label>
          <label class="space-y-1">
            <span class="text-xs text-[var(--color-ide-text-dim)]">语速: {{ ttsSpeed }}x</span>
        ...
        <div v-if="ttsResult" class="bg-gray-800/50 rounded-xl p-4 flex items-center justify-between">
          <span class="text-xs text-[var(--color-ide-text-dim)]">时长 {{ ttsResult.duration_seconds.toFixed(1) }}s</span>
      ...
      <div class="bg-gray-900/50 rounded-2xl p-6 border border-gray-800/50 space-y-4">
        ...
        <label class="block">
          <span class="text-xs text-[var(--color-ide-text-dim)] mb-2 block">上传音频文件</span>
          <div class="flex items-center gap-3 border-2 border-dashed border-gray-700/50 rounded-xl p-6 text-center hover:border-purple-500/30 transition cursor-pointer">
            <Upload class="w-5 h-5 text-[var(--color-ide-text-dim)]" />
            <span class="text-sm text-[var(--color-ide-text-dim)]">{{ sttFile ? sttFile.name : '点击选择 mp3/wav/flac/ogg' }}</span>
        ...
        <div class="flex items-center gap-3">
          ...
          <span class="text-xs text-[var(--color-ide-text-dim)]">或通过 URL</span>
          <div class="flex-1 h-px bg-gray-700/50" />
        </div>

        <div class="flex gap-2">
          <input v-model="sttUrl" class="flex-1 bg-gray-800 border border-gray-700/50 rounded-xl px-3 py-2 text-sm text-[var(--color-ide-text)] outline-none" placeholder="https://..." />
          <button @click="doSTTUrl" :disabled="sttLoading || !sttUrl.trim()" class="px-4 py-2 rounded-xl bg-gray-800 hover:bg-gray-700 text-sm text-[var(--color-ide-text)] transition">转录</button>
        </div>

        <div v-if="sttResult" class="bg-gray-800/50 rounded-xl p-4 space-y-2">
          <p class="text-sm text-[var(--color-ide-text)] leading-relaxed">{{ sttResult.text }}</p>
          <div v-if="sttResult.segments?.length" class="text-xs text-[var(--color-ide-text-dim)]">
