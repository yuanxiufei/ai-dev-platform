  ...
  <div class="p-6 max-w-5xl mx-auto space-y-6">
    <header class="flex items-center justify-between">
      <div>
        ...
        <p class="text-[var(--color-ide-text-dim)] mt-2">Token · 延迟 · 成本实时监控</p>
      </div>
      <div class="flex gap-2">
        <button v-for="d in [1, 7, 30]" :key="d" @click="period = d; fetchAll()"
          :class="['px-3 py-1.5 rounded-xl text-sm transition-colors', period === d ? 'bg-brand-500/15 text-brand-400' : 'text-[var(--color-ide-text-dim)] hover:text-white hover:bg-gray-800']">{{ d === 1 ? '今天' : d === 7 ? '7天' : '30天' }}</button>
      </div>
    </header>

    <div v-if="loading" class="text-center py-16 text-[var(--color-ide-text-dim)]">加载中...</div>
    <template v-else>
      <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div class="p-4 rounded-2xl bg-gray-900/50 border border-gray-800/50 text-center">
          ...
          <span class="text-xs text-[var(--color-ide-text-dim)]">总调用</span>
        </div>
        <div class="p-4 rounded-2xl bg-gray-900/50 border border-gray-800/50 text-center">
        ...
        <div class="p-4 rounded-2xl bg-gray-900/50 border border-gray-800/50 text-center">
          ...
          <span class="text-xs text-[var(--color-ide-text-dim)]">总 Token</span>
        </div>
        <div class="p-4 rounded-2xl bg-gray-900/50 border border-gray-800/50 text-center">
          ...
          <span class="text-xs text-[var(--color-ide-text-dim)]">估算成本</span>
        </div>
        <div class="p-4 rounded-2xl bg-gray-900/50 border border-gray-800/50 text-center">
          ...
          <span class="text-xs text-[var(--color-ide-text-dim)]">平均延迟</span>
        </div>
      </div>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-gray-800">
                <th class="text-left text-[var(--color-ide-text-dim)] font-medium py-2 px-3">模型</th>
                <th class="text-left text-[var(--color-ide-text-dim)] font-medium py-2 px-3">调用</th>
                <th class="text-left text-[var(--color-ide-text-dim)] font-medium py-2 px-3">Token</th>
                <th class="text-left text-[var(--color-ide-text-dim)] font-medium py-2 px-3">延迟</th>
                <th class="text-left text-[var(--color-ide-text-dim)] font-medium py-2 px-3">成本</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="m in modelStats" :key="m.model_name" class="border-b border-gray-800/30">
                ...
                <td class="py-2 px-3 text-[var(--color-ide-text-dim)]">{{ m.calls }}</td>
                <td class="py-2 px-3 text-[var(--color-ide-text-dim)]">{{ m.total_tokens?.toLocaleString() }}</td>
                <td class="py-2 px-3 text-[var(--color-ide-text-dim)]">{{ m.avg_latency_ms?.toFixed(0) }}ms</td>
      ...
      <div v-if="trends.length" class="bg-gray-900/50 rounded-2xl p-6 border border-gray-800/50">
        ...
        <div class="space-y-2">
          <div v-for="t in trends" :key="t.time" class="flex items-center gap-3">
            <div class="w-16 text-xs text-[var(--color-ide-text-dim)] shrink-0">{{ t.time.slice(5, 10) }}</div>
            ...
            <div class="w-10 text-xs text-[var(--color-ide-text-dim)] text-right shrink-0">{{ t.calls }}</div>
          </div>
        </div>
      </div>
    </template>
