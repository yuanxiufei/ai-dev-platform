  ...
  <div class="p-6">
    <div class="max-w-5xl mx-auto">
      <header class="mb-8 text-center">
        ...
        <p class="text-[var(--color-ide-text-dim)] mt-2">并排对比两个 AI 模型，投票选出最佳回答</p>
      </header>
      ...
      <div class="p-6 rounded-2xl bg-gray-900/50 border border-gray-800/50 mb-8">
        <div class="flex flex-col sm:flex-row gap-4 mb-4">
          <div class="flex-1">
            <label class="text-xs text-[var(--color-ide-text-dim)] mb-1 block">模型 A</label>
          ...
          <div class="flex items-end pb-2">
            <span class="text-2xl font-bold text-[var(--color-ide-text-dim)]">VS</span>
          </div>
          <div class="flex-1">
            <label class="text-xs text-[var(--color-ide-text-dim)] mb-1 block">模型 B</label>
        ...
        <textarea v-model="form.prompt" rows="4" class="w-full px-4 py-3 rounded-xl bg-gray-800 border border-gray-700 text-sm text-white placeholder-[var(--color-ide-text-dim)] outline-none focus:border-brand-500/50 resize-none mb-4" placeholder="输入你想对比的问题..."></textarea>
      ...
      <div v-if="result" class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div class="p-6 rounded-2xl bg-gray-900/50 border border-gray-800/50">
          <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs text-[var(--color-ide-text-dim)]">{{ result.latency_a_ms?.toFixed(0) }}ms</span>
          </div>
          <div class="text-sm text-[var(--color-ide-text)] leading-relaxed whitespace-pre-wrap max-h-96 overflow-y-auto">{{ result.response_a }}</div>
        </div>
        <div class="p-6 rounded-2xl bg-gray-900/50 border border-gray-800/50">
          <div class="flex items-center justify-between mb-4">
            <span class="px-2.5 py-0.5 text-xs rounded-full font-medium bg-purple-500/15 text-purple-400">{{ result.model_b }}</span>
            <span class="text-xs text-[var(--color-ide-text-dim)]">{{ result.latency_b_ms?.toFixed(0) }}ms</span>
          </div>
          <div class="text-sm text-[var(--color-ide-text)] leading-relaxed whitespace-pre-wrap max-h-96 overflow-y-auto">{{ result.response_b }}</div>
        </div>
      </div>

      <!-- 投票 -->
      <div v-if="result && !voted" class="p-4 rounded-2xl bg-gray-900/50 border border-gray-800/50 text-center mb-8">
        <p class="text-[var(--color-ide-text-dim)] mb-4">你认为谁的回答更好？</p>
        <div class="flex justify-center gap-4">
          ...
          <button @click="vote('tie')" class="px-6 py-3 rounded-xl text-sm font-medium bg-gray-500/10 text-[var(--color-ide-text-dim)] border border-gray-500/20 hover:bg-gray-500/20 transition-all hover:scale-105">🤝 平局</button>
        </div>
      </div>
      ...
      <div class="p-6 rounded-2xl bg-gray-900/50 border border-gray-800/50">
        ...
        <div v-if="!rankings.length" class="text-center text-[var(--color-ide-text-dim)] py-8">暂无数据</div>
        <div v-else class="space-y-1">
          <div v-for="(r, i) in rankings" :key="r.model_name" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-800/50">
            <span class="w-7 h-7 flex items-center justify-center rounded-full text-xs font-bold bg-gray-800 text-[var(--color-ide-text-dim)]" :class="{ 'bg-yellow-500/20 text-yellow-400': i === 0, 'bg-gray-300/20 text-[var(--color-ide-text)]': i === 1, 'bg-orange-500/20 text-orange-400': i === 2 }">{{ i + 1 }}</span>
            <span class="flex-1 text-sm text-white">{{ r.model_name }}</span>
            <span class="text-sm font-bold text-brand-400">{{ Math.round(r.elo) }}</span>
            <span class="text-xs text-[var(--color-ide-text-dim)] w-20 text-right">{{ r.wins }}W / {{ r.losses }}L</span>
          </div>
        </div>
      </div>
    </div>
