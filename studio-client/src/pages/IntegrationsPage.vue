  ...
  <div class="h-full flex flex-col">
    ...
    <div class="flex items-center justify-between px-8 pt-8 pb-6 border-b border-[var(--color-ide-border)]">
      <div>
        ...
        <p class="text-sm text-[var(--color-ide-text-dim)] mt-1.5 ml-[3.25rem]">连接后端云服务和外部工具，扩展平台能力</p>
      </div>
    ...
    <div class="flex-1 flex overflow-hidden">
      <div class="w-52 shrink-0 border-r border-[var(--color-ide-border)] p-4 space-y-1">
        <p class="text-[11px] font-medium text-[var(--color-ide-text-dim)] uppercase tracking-wider px-2 mb-3">服务分类</p>
        <button
          v-for="cat in categories" :key="cat.id"
          @click="activeCategory = cat.id"
          :class="['w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all duration-200 text-left', activeCategory === cat.id ? 'bg-brand-500/10 text-brand-400 font-medium' : 'text-[var(--color-ide-text-dim)] hover:text-[var(--color-ide-text)] hover:bg-[var(--color-ide-surface-hover)]']"
        >
          <component :is="cat.icon" class="w-4 h-4 shrink-0" />
          {{ cat.label }}
          <span class="ml-auto text-xs text-[var(--color-ide-text-dim)]">{{ cat.id === 'all' ? integrations.length : integrations.filter(i => i.category === cat.id).length }}</span>
        </button>
        ...
        <div class="mt-6 pt-4 border-t border-[var(--color-ide-border)]">
          <p class="text-[10px] text-[var(--color-ide-text-dim)] mb-2 px-2">快速注册</p>
          <div class="space-y-0.5">
            <button
              v-for="p in presetIntegrations.filter(pi => !integrations.find(i => i.name === pi.name))"
              :key="p.name"
              @click="usePreset(p)"
              class="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs text-[var(--color-ide-text-dim)] hover:text-[var(--color-ide-text)] hover:bg-[var(--color-ide-surface-hover)] transition-colors text-left"
            >
              <span>{{ p.icon }}</span>
      ...
      <div class="flex-1 overflow-y-auto p-6">
        ...
        <div v-else-if="!integrations.length" class="flex flex-col items-center justify-center py-24 text-[var(--color-ide-text-dim)]">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          <div
            ...
            :key="item.id"
            :class="['group rounded-2xl border p-5 transition-all duration-200 flex flex-col', item.connected ? 'border-green-500/15 bg-green-500/[0.03] hover:border-green-500/30' : 'border-[var(--color-ide-border)] bg-[var(--color-ide-surface-hover)] hover:border-[var(--color-ide-border)] hover:bg-[var(--color-ide-surface-hover)]']"
          >
            <div class="flex items-center gap-3 mb-3">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="text-[11px] text-[var(--color-ide-text-dim)] mt-0.5">
            ...
            <p class="text-xs text-[var(--color-ide-text-dim)] leading-relaxed mb-4 flex-1">{{ item.description || '无描述' }}</p>

            <div class="flex items-center gap-2 pt-3 border-t border-[var(--color-ide-border)]">
              <button
              ...
              <button @click="showConfig = item" class="p-2 rounded-lg text-[var(--color-ide-text-dim)] hover:text-[var(--color-ide-text-dim)] hover:bg-[var(--color-ide-surface-hover)] transition-colors" title="查看详情">
                <Settings2 class="w-3.5 h-3.5" />
              </button>
              <button @click="handleDelete(item.id)" class="p-2 rounded-lg text-[var(--color-ide-text-dim)] hover:text-red-400 hover:bg-red-500/10 transition-colors opacity-0 group-hover:opacity-100" title="删除">
    ...
    <Teleport to="body">
      <div v-if="showCreate" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" @click.self="showCreate = false">
        <div class="bg-surface-900 rounded-2xl border border-[var(--color-ide-border)] w-[600px] max-h-[85vh] flex flex-col shadow-2xl">
          ...
          <div class="flex-1 overflow-y-auto p-5 space-y-4">
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-medium text-[var(--color-ide-text-dim)] mb-1.5">标识 <span class="text-red-400">*</span></label>
                <input v-model="form.name" placeholder="supabase" class="w-full rounded-xl border border-[var(--color-ide-border)] bg-[var(--color-ide-surface-hover)] px-4 py-2.5 text-sm font-mono text-white placeholder-[var(--color-ide-text-dim)] focus:outline-none focus:border-brand-500/40 transition-all" />
              </div>
              <div>
                <label class="block text-xs font-medium text-[var(--color-ide-text-dim)] mb-1.5">显示名称 <span class="text-red-400">*</span></label>
                <input v-model="form.display_name" placeholder="Supabase" class="w-full rounded-xl border border-[var(--color-ide-border)] bg-[var(--color-ide-surface-hover)] px-4 py-2.5 text-sm text-white placeholder-[var(--color-ide-text-dim)] focus:outline-none focus:border-brand-500/40 transition-all" />
              </div>
            </div>
            <div>
              <label class="block text-xs font-medium text-[var(--color-ide-text-dim)] mb-1.5">描述</label>
              <input v-model="form.description" placeholder="PostgreSQL 数据库、认证、存储一体化后端" class="w-full rounded-xl border border-[var(--color-ide-border)] bg-[var(--color-ide-surface-hover)] px-4 py-2.5 text-sm text-white placeholder-[var(--color-ide-text-dim)] focus:outline-none focus:border-brand-500/40 transition-all" />
            </div>
            <div>
              <label class="block text-xs font-medium text-[var(--color-ide-text-dim)] mb-1.5">分类</label>
              <select v-model="form.category" class="w-full rounded-xl border border-[var(--color-ide-border)] bg-[var(--color-ide-surface-hover)] px-4 py-2.5 text-sm text-white focus:outline-none focus:border-brand-500/40 transition-all">
            ...
            <div class="pt-2 border-t border-[var(--color-ide-border)]">
              <p class="text-[11px] font-medium text-[var(--color-ide-text-dim)] mb-3">连接凭据</p>
              <div class="grid grid-cols-1 gap-3">
                <div>
                  <label class="block text-[10px] text-[var(--color-ide-text-dim)] mb-1">API Key</label>
                  <input v-model="formApiKey" type="password" placeholder="••••••••" class="w-full rounded-xl border border-[var(--color-ide-border)] bg-[var(--color-ide-surface-hover)] px-4 py-2.5 text-sm text-white placeholder-[var(--color-ide-text-dim)] focus:outline-none focus:border-brand-500/40 transition-all" />
                </div>
                <div>
                  <label class="block text-[10px] text-[var(--color-ide-text-dim)] mb-1">URL / Endpoint</label>
                  <input v-model="formUrl" placeholder="https://api.example.com" class="w-full rounded-xl border border-[var(--color-ide-border)] bg-[var(--color-ide-surface-hover)] px-4 py-2.5 text-sm font-mono text-white placeholder-[var(--color-ide-text-dim)] focus:outline-none focus:border-brand-500/40 transition-all" />
                </div>
                <div>
                  <label class="block text-[10px] text-[var(--color-ide-text-dim)] mb-1">Token / Secret</label>
                  <input v-model="formToken" type="password" placeholder="••••••••" class="w-full rounded-xl border border-[var(--color-ide-border)] bg-[var(--color-ide-surface-hover)] px-4 py-2.5 text-sm text-white placeholder-[var(--color-ide-text-dim)] focus:outline-none focus:border-brand-500/40 transition-all" />
                </div>
              </div>
            </div>
          </div>
    ...
    <Teleport to="body">
      <div v-if="showConfig" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" @click.self="showConfig = null">
        <div class="bg-surface-900 rounded-2xl border border-[var(--color-ide-border)] w-[500px] max-h-[70vh] flex flex-col shadow-2xl">
          <div class="flex items-center justify-between p-5 border-b border-[var(--color-ide-border)]">
            ...
            <button @click="showConfig = null" class="p-1.5 rounded-lg hover:bg-[var(--color-ide-surface-hover)] text-[var(--color-ide-text-dim)] transition-colors"><X class="w-4 h-4" /></button>
          </div>
          <div class="flex-1 overflow-y-auto p-5 space-y-3">
            <div class="flex justify-between text-sm">
              <span class="text-[var(--color-ide-text-dim)]">状态</span>
              <span :class="showConfig.connected ? 'text-green-400' : 'text-[var(--color-ide-text-dim)]'">{{ showConfig.connected ? '已连接' : '未连接' }}</span>
            </div>
            <div class="flex justify-between text-sm">
              <span class="text-[var(--color-ide-text-dim)]">分类</span>
              <span class="text-[var(--color-ide-text)]">{{ showConfig.category }}</span>
            </div>
            <div v-if="showConfig.config" class="pt-2 border-t border-[var(--color-ide-border)]">
              <p class="text-[11px] text-[var(--color-ide-text-dim)] mb-2">配置信息</p>
              <pre class="text-xs text-[var(--color-ide-text-dim)] whitespace-pre-wrap font-mono bg-[var(--color-ide-surface-hover)] p-3 rounded-xl">{{ JSON.stringify(showConfig.config, null, 2) }}</pre>
            </div>
          </div>
        </div>
      </div>
