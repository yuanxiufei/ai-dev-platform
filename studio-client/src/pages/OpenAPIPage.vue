  ...
  <div class="p-6 max-w-5xl mx-auto space-y-6">
    <header class="flex items-center justify-between">
      <div>
        ...
        <p class="text-[var(--color-ide-text-dim)] mt-2">自动发现 OpenAPI 端点并调用</p>
      </div>
    ...
    <div v-if="loading" class="text-center py-12"><RefreshCw class="w-6 h-6 animate-spin text-[var(--color-ide-text-dim)] mx-auto" /></div>
    <div v-else-if="!servers.length" class="text-center py-16 text-[var(--color-ide-text-dim)] bg-gray-900/50 rounded-2xl border border-gray-800/50">
      <Globe class="w-10 h-10 mx-auto mb-3 text-[var(--color-ide-text-dim)]" />
    ...
    <div v-else class="flex gap-4">
      <div class="w-56 shrink-0 space-y-1">
        <button v-for="svr in servers" :key="svr.id" @click="viewTools(svr.id)"
          :class="['w-full text-left px-3 py-2.5 rounded-xl transition text-sm', selectedServerId === svr.id ? 'bg-brand-500/15 text-brand-400' : 'bg-gray-800/50 hover:bg-gray-800 text-[var(--color-ide-text)]']">
          <div class="font-medium truncate">{{ svr.name }}</div>
          <div class="text-xs text-[var(--color-ide-text-dim)]">{{ svr.tool_count }} tools</div>
        </button>
      </div>
      <div class="flex-1">
        <div v-if="!selectedServerId" class="text-center py-16 text-[var(--color-ide-text-dim)]">选择左侧服务器查看工具</div>
        <div v-else-if="!tools.length" class="text-center py-16 text-[var(--color-ide-text-dim)]">此服务器无工具</div>
        <div v-else class="space-y-2">
          <div v-for="tool in tools" :key="tool.function.name" class="flex items-center gap-3 p-3 rounded-xl bg-gray-900/50 border border-gray-800/50 hover:border-brand-500/30 transition group">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-[var(--color-ide-text-dim)] line-clamp-1">{{ tool.function.description || '无描述' }}</p>
            </div>
            <button @click="openToolCall(tool)" class="px-2 py-1 rounded-lg text-xs bg-gray-800 hover:bg-gray-700 text-[var(--color-ide-text)] transition opacity-0 group-hover:opacity-100">
    ...
    <Teleport to="body">
      <div v-if="showDiscover" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60" @click.self="showDiscover = false">
        <div class="bg-gray-900 rounded-2xl border border-gray-800 w-[480px] p-6 space-y-4">
          ...
          <div><label class="block text-xs text-[var(--color-ide-text-dim)] mb-1">Spec URL *</label><input v-model="discoverUrl" placeholder="https://api.example.com/openapi.json" class="w-full px-3 py-2 bg-gray-800 border border-gray-700/50 rounded-xl text-sm text-white font-mono outline-none" /></div>
          <div><label class="block text-xs text-[var(--color-ide-text-dim)] mb-1">名称（可选）</label><input v-model="discoverName" class="w-full px-3 py-2 bg-gray-800 border border-gray-700/50 rounded-xl text-sm text-white outline-none" /></div>
      ...
      <div v-if="showToolCall" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60" @click.self="showToolCall = false">
        <div class="bg-gray-900 rounded-2xl border border-gray-800 w-[520px] max-h-[80vh] flex flex-col">
          <div class="flex items-center justify-between p-4 border-b border-gray-800">
            ...
            <button @click="showToolCall = false" class="text-[var(--color-ide-text-dim)]">✕</button>
          </div>
          <div class="p-4 space-y-3 overflow-auto">
            <div><label class="block text-xs text-[var(--color-ide-text-dim)] mb-1">参数 (JSON)</label><textarea v-model="callArgs" rows="5" class="w-full px-3 py-2 bg-gray-800 border border-gray-700/50 rounded-xl text-sm text-white font-mono outline-none resize-none" /></div>
            ...
            <pre v-if="callResult" class="p-3 bg-gray-800 rounded-xl text-xs text-[var(--color-ide-text)] whitespace-pre-wrap font-mono max-h-64 overflow-auto">{{ JSON.stringify(callResult, null, 2) }}</pre>
          </div>
        </div>
      </div>
    </Teleport>
