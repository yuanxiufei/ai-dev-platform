...
const fs = {
  readDirectory: (path: string, options?: any) =>
    invoke<FileInfo[]>("read_directory", { request: { path, ...options } }),
  readFile: (path: string) => invoke<string>("read_file", { path }),
  writeFile: (path: string, content: string) =>
    invoke<void>("write_file", { path, content }),
  createFile: (path: string) => invoke<FileInfo>("create_file", { path }),
  createDirectory: (path: string) =>
    invoke<FileInfo>("create_directory", { path }),
  deletePath: (path: string, recursive?: boolean) =>
    invoke<void>("delete_path", { path, recursive }),
  renamePath: (oldPath: string, newPath: string) =>
    invoke<FileInfo>("rename_path", { oldPath, new_path: newPath }),
  copyPath: (src: string, dst: string) =>
    invoke<FileInfo>("copy_path", { src, dst }),
  searchFiles: (params: any) =>
    invoke<string[]>("search_files", { request: params }),
  searchInFile: (params: any) =>
    invoke<SearchResult[]>("search_in_file", { request: params }),
  getFileInfo: (path: string) => invoke<FileInfo>("get_file_info", { path }),
}

const terminal = {
  spawnShell: (config?: any) =>
    invoke<ProcessInfo>("spawn_shell", { config: config || {} }),
  writeToPty: (pid: number, data: string) =>
    invoke<void>("write_to_pty", { pid, data }),
  resizePty: (pid: number, cols: number, rows: number) =>
    invoke<void>("resize_pty", { pid, cols, rows }),
...
const git = {
  ...
  getLog: (path: string, limit?: number) =>
    invoke<GitCommitInfo[]>("git_log", { path, limit }),
  commit: (path: string, message: string, files?: string[]) =>
    invoke<string>("git_commit", { path, message, files }),
  listBranches: (path: string) => invoke<GitBranch[]>("git_branch", { path }),
  checkout: (path: string, branch: string, createNew?: boolean) =>
    invoke<void>("git_checkout", { path, branch, create_new: createNew }),
  push: (path: string, remote?: string, branch?: string) =>
    invoke<string>("git_push", { path, remote, branch }),
  pull: (path: string, remote?: string, branch?: string) =>
    invoke<string>("git_pull", { path, remote, branch }),
}
...
const plugins = {
  ...
  callCommand: (pluginId: string, command: string, args?: any) =>
    invoke<any>("call_plugin_command", {
      request: { pluginId, command, args },
    }),
}
...
const events = {
  onFileWatchEvent: (cb: (e: any) => void): Promise<UnlistenFn> =>
    listen("file-watch-event", cb),
}
  ...
  window: {
    toggleFullscreen: () =>
      getCurrentWindow().then((w) =>
        invoke<void>("toggle_fullscreen", { window: w }),
      ),
  },
  plugins,
  settings,
