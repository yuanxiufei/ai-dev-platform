...
<template>
  <div class="run-dashboard rounded-xl border border-white/[0.06] bg-white/[0.01] overflow-hidden">
    <!-- 头部概览 -->
    <div class="grid grid-cols-4 divide-x divide-white/[0.04] border-b border-white/[0.04]">
      ...
      <div class="flex flex-col items-center justify-center py-3 px-2">
        <span class="text-[10px] text-gray-500 uppercase tracking-wider mb-1">轮次</span>
        <span class="text-lg font-bold text-white tabular-nums">
          <span v-if="currentTurn">{{ currentTurn }}</span>
          <span v-else>0</span>
        </span>
        <span class="text-[10px] text-gray-600">/ {{ maxTurns || '?' }}</span>
      </div>

      <!-- 工具调用 -->
      <div class="flex flex-col items-center justify-center py-3 px-2">
        <span class="text-[10px] text-gray-500 uppercase tracking-wider mb-1">工具</span>
      ...
      <!-- 延迟 -->
      <div class="flex flex-col items-center justify-center py-3 px-2">
        <span class="text-[10px] text-gray-500 uppercase tracking-wider mb-1">延迟</span>
        <span class="text-lg font-bold text-white tabular-nums">
          {{ totalLatencyMs ? (totalLatencyMs / 1000).toFixed(1) : '--' }}
        </span>
        <span class="text-[10px] text-gray-600">{{ totalLatencyMs ? totalLatencyMs + 'ms' : '等待中' }}</span>
      </div>

      <!-- 模型 -->
      <div class="flex flex-col items-center justify-center py-3 px-2">
        <span class="text-[10px] text-gray-500 uppercase tracking-wider mb-1">模型</span>
        <div class="flex items-center gap-1">
          <Cpu class="w-3.5 h-3.5 text-brand-400" />
          <span class="text-xs font-mono text-gray-300 truncate max-w-[80px]">
            {{ modelUsed || '--' }}
          </span>
        </div>
      ...
      >
        ...
        >
          <div
            ...
            :class="turn.finalOutput ? 'bg-emerald-500/15 text-emerald-400' : 'bg-white/[0.03] text-gray-500'"
          >
            {{ turn.turn }}
          </div>
          ...
          <div class="flex-1 min-w-0 text-left">
            ...
            <div class="flex items-center gap-2 mt-0.5">
              <div class="flex items-center gap-1">
                <Wrench class="w-2.5 h-2.5 text-gray-600" />
                <span class="text-[10px] text-gray-600">{{ turn.toolCalls.length }} tools</span>
              </div>
              <div v-if="turn.latencyMs" class="flex items-center gap-1">
                <Timer class="w-2.5 h-2.5 text-gray-600" />
                <span class="text-[10px] text-gray-600">{{ turn.latencyMs }}ms</span>
              </div>
            </div>
          </div>

          <span class="text-gray-600 text-xs transition-transform duration-200" :class="{ 'rotate-180': selectedTurn === turn.turn }">▾</span>
        </button>
        ...
        <div v-if="selectedTurn === turn.turn" class="px-4 pb-3 space-y-2 border-t border-white/[0.03]">
          ...
          <div v-if="turn.reasoningSteps.length > 0" class="pt-2">
            <p class="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1">推理过程</p>
            <div class="space-y-1">
              <div
                v-for="(step, si) in turn.reasoningSteps"
                :key="si"
                class="flex items-start gap-1.5 text-[10px] text-gray-400"
              >
          ...
          <div v-if="turn.toolCalls.length > 0">
            <p class="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1">工具调用</p>
            <div class="space-y-1">
              <div
              ...
              >
                ...
                />
                <span class="font-mono text-gray-300 truncate flex-1">{{ tc.name }}</span>
                <span v-if="tc.result" class="text-[10px] text-gray-500 truncate max-w-[100px]">
          ...
          <div v-if="turn.finalOutput" class="pt-1">
            <p class="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1">输出</p>
