...
<template>
  <div class="h-[var(--tabbar-height)] flex items-end bg-[var(--color-tab-inactive)] border-b border-[var(--color-tab-border)] shrink-0 overflow-x-auto overflow-y-hidden" @dragover="onDragOver">
    <div class="shrink-0 h-full flex items-center px-3 text-[11px] text-[var(--color-ide-text-dim)] border-r border-[var(--color-tab-border)] bg-[var(--color-ide-bg)] cursor-default select-none whitespace-nowrap">
      <span v-if="store.activeTab?.filePath" class="flex items-center gap-1">
        <span v-for="(seg, i) in store.activeTab.filePath.split(/[/\\]/)" :key="i" class="flex items-center gap-1">
          <ChevronRight v-if="i > 0" :size="10" class="opacity-40" /><span :class="{ 'text-[var(--color-ide-text)]': i === store.activeTab.filePath.split(/[/\\]/).length - 1 }">{{ seg }}</span>
        </span>
      </span>
    ...
    <div class="flex h-full items-stretch">
      <div v-for="tab in store.sortedTabs" :key="tab.id" draggable="true"
        class="group/tab relative flex items-center gap-1.5 pl-3 pr-1.5 h-full min-w-[100px] max-w-[180px] cursor-pointer border-r border-[var(--color-tab-border)] transition-colors select-none shrink-0"
        ...
        <Circle v-if="tab.modified" :size="8" :fill="tab.id === store.activeTabId ? '#f5e0dc' : '#939ab7'" class="shrink-0 mt-0.5" :stroke="tab.id === store.activeTabId ? '#f5e0dc' : '#939ab7'" stroke-width="0.5" />
        <span class="truncate flex-1 text-[11px] leading-none">{{ tab.label }}</span>
        <button class="opacity-0 group-hover/tab:opacity-100 group-focus-within/tab:opacity-100 transition-opacity p-0.5 rounded hover:bg-white/10 shrink-0" :class="{ '!opacity-100': tab.id === store.activeTabId }" title="关闭" @click.stop="store.closeTab(tab.id)"><X :size="14" /></button>
        <div v-if="tab.id === store.activeTabId" class="absolute bottom-0 left-0 right-0 h-0.5 bg-[var(--color-ide-accent)]" />
      </div>
      <div v-if="store.sortedTabs.length > 0" class="flex items-center justify-center px-4 text-[10px] text-[var(--color-ide-text-dim)] hover:text-[var(--color-ide-text)] cursor-pointer transition-colors shrink-0">Review Next File →</div>
    </div>
  </div>
</template>
