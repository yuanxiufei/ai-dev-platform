...
import { Play, Trash2, Clock, Filter, RefreshCw } from 'lucide-vue-next'
import { useVideoStore, type VideoItem, type TaskStatus } from '../../stores/videoStore'
...
const filtered = computed(() =>
  filterStatus.value === 'all'
    ? store.videos
    : store.videos.filter(v => v.status === filterStatus.value),
)
...
<template>
  <div class="py-12 animate-fade-in-up">
    <div class="flex items-center justify-between mb-8">
      <div><h1 class="text-3xl font-bold text-white">作品画廊</h1><p class="text-gray-400 text-sm mt-1">你的 AI 生成短视频</p></div>
      <button @click="store.fetchMyTasks()" class="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/[0.03] border border-white/[0.06] text-gray-400 hover:text-white hover:border-white/15 transition-all text-sm" :disabled="store.isLoading">
    ...
    <div v-if="store.isLoading" class="flex items-center justify-center py-20">
      <RefreshCw class="w-8 h-8 text-violet-400 animate-spin" />
    </div>
    <!-- empty -->
    ...
    <div v-else class="grid grid-cols-4 gap-4">
      <div v-for="v in filtered" :key="v.taskId" @click="v.status === 'completed' && router.push(`/play/${v.taskId}`)"
        ...
        <div class="aspect-[9/16] bg-gray-800 flex items-center justify-center relative">
          <img v-if="getThumbnail(v)" :src="getThumbnail(v)!" :alt="v.title" class="w-full h-full object-cover" loading="lazy" />
        ...
        <div class="p-3 space-y-1">
          ...
          <div class="flex items-center gap-2 text-xs text-gray-500"><Clock class="w-3 h-3" /><span>{{ v.duration }}s</span><span class="text-gray-600">·</span><span>{{ v.style || v.modelName }}</span></div>
        </div>
        <button @click.stop="store.removeVideo(v.taskId)" class="absolute top-2 left-2 p-1.5 rounded-lg bg-black/40 text-gray-400 opacity-0 group-hover:opacity-100 transition-all hover:text-red-400 hover:bg-black/60">
